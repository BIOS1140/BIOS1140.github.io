...
::: {.green}
**Important concept:**  
_Green boxes summarizes important concepts from the text._
:::
...
geno_freq <- pivot_longer(geno_freq, names_to = "genotype", values_to = "freq", -p, -q)
```
...
$\displaystyle X^2 = \sum_{i=1}^k \frac{(E_i - O_i)^2}{E_i}$
...
Using the dollar sign notation we learned about in [Chapter 1](https://evolutionarygenetics.github.io/Introduction.html), we can extract any of these at will. For example `mychi$expected` will produce the expected values whereas `mychi$statistic` will produce the chi-squared value.
...
To simulate drift, we need to allow individuals to mate at random. In the book you will recall we used a coin-flipping experiment to sample our alleles to form genotypes, since both alleles have a frequency of 0.5. This is essentially a form of **binomial sampling**. We can do this kind of sampling in R with the `rbinom()` function.

The `rbinom()` function essentially simulates a coin flip. However, while in a regular coin flip the probability of heads and tails is always 0.5, `rbinom()` allows for changing these probabilities. Simplified, the following code says: "flip 1 coin (`n`) 16 times (`size`) with a probability of 0.5 (`prob`) for getting heads". The output is the number of heads we got.
...
In our example, the output of `rbinom` could represent the number of $A_1$ alleles in the next generation -- so we could easily work out $p´$ and $q´$, i.e. $p$ and $q$ in the next generation). We can use the following code to calculate the number of $A_1$ in generation 2:
...
### Wrapping the simulation in a function (advanced)

_The code in this section contains creating your own custom functions, which you won't learn about until next week. This code is marked with a yellow box, and don't worry if you don't understand it yet, hopefully you will next week!_

For your convenience, we have wrapped our simulation in a custom function. Run the code in the yellow box to be able to use the function.

:::{.yellow}

**Advanced code**

_You need to run the code below in order to complete the tutorial, but you don't need to understand it (yet)!_

```{r sim-fun}
drift_sim <- function(N, p_init, ngen){
  # create vector for storing results
  p <- rep(NA, ngen)
  # set first element to initial p
  p[1] <- p_init
...
After running the code above, you can run the same simulation as earlier using our new `drift_sim()` function like this:

```{r}
drift_sim(N = 8, p_init = 0.5, ngen = 100)
```

### Visualizing drift

Now that we have a function to simulate data across generations, we can easily do this for different population sizes. Let's simulate 1000 generations of drift for three populations of 10, 100 and 1000 individuals. We'll keep the initial allele frequency, `p` at 0.5 in all cases. We will then combine them all into a `data.frame` alongside the number of generations.

```{r, eval = TRUE, echo = TRUE, results = "hide"}
# simulate drift for three different population sizes
n10 <- drift_sim(N = 10, p_init = 0.5, ngen = 1000)
n100 <- drift_sim(N = 100, p_init = 0.5, ngen = 1000)
n1000 <- drift_sim(N = 1000, p_init = 0.5, ngen = 1000)
