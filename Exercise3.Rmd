...
In addition to the HW model and testing for deviations from HWE, we will also learn how to simulate genetic drift in R. Here we will make use of the visualisation skills we learned with ggplot2 in Chapter 2 to recreate Figure 3.7 from the textbook. We hope that this will have the dual benefit of letting you play with population parameters in order to understand genetic drift but also to help you develop your R programming skills in more detail. During the course of this session we will encounter how to write for loops, learn about vectorization and also learn a little bit about how to write your own custom functions.

In this tutorial we will be exploring the Hardy-Weinberg model (HW-model). The model is used as a null hypotheses to investigate whether a population is affected by demographic or evolutionary processes. It simply states that a population is in HW-equilibrium if one is able to predict genotype frequencies from allele frequencies, or in other words: allele-and genotype frequencies remain constant in the subsequent generation.
...
2.  A section focused on using R for evolutionary biology.
...
How this would look if done manually is included in the footnotes of this document[^4].

[^4]:
    This is how the for-loop with `friends` work:
    ```{r, eval = FALSE}
...
To remind you of where we started: we want to add the adjacent elements of our vector `x` together in the smartest way possible. Below is how this can be done with for-loops. I encourage you to try to solve it yourself before looking at the solution.

Solution: We loop over the vector `2:length(x)`[^5]. For every round, we add `x[index]` and `x[index - 1]` and print this.
