---
title: "Inferring evolutionary processes from sequence data"
output: 
...
First of all, let's turn our attention to the number of segregating sites. How does using different numbers of sequences effect this? We can loop through different values of the maximum number of sequences. So we will first use 2 sequences, then 3, then 4 and so on, until we use all 15.[^1]

[^1]: Take a look at the code and see if you understand the following: why did we initialise the `ss` vector with 14 values, not 15? Why do we use `i+1` for subsetting the `woodmouse` data? Are there other ways we could have done this?
