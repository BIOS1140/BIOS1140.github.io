...
Once it is installed, we will clear the R environment with `rm(list = ls())`[^1] and then load the two packages.
...
[^1]: Note that clearing the R environment when you start a script is good practice to make sure you don't have any conflicts with previously loaded data.
...
There appears to be a difference in petal length between species. We could also look at the median and standard deviation to find out more, see footnotes[^2].

[^2]: 
    We calculate median and standard deviations using multiple summaries within a single `summarise()`. Also, remember that the name you give the arguments within `summarise()` will be the column names in your new data frame.
...
* The boundaries of the box are the 1st and 3rd quartile, i.e., half of your data is contained within the box[^3]
* The lines outside the box are the range of non-outlier data, and the points are outliers


A boxplot is good at showing the distribution in the data. From this plot, we can see that the species obviously differ in this trait. But how large differences do we need to make confident conclusions? This is where statistic testing comes in[^4].
...
[^4]: Like in Section 2.2 in week 3, where we tested for deviations from the Hardy-Weinberg expectation with a Chi-squared test.
...
Above, we used a formula as our first argument `Petal.Length ~ Species`[^5]. This is essentially like saying, "How does petal length vary with species"? This part of our argument to `aov` specifies what we are actually testing. We also added `data = iris` to specify that we are using the `iris` data for this calculation.

[^5]: The character `~` is called "tilde", in case you want to google how to produce it on your keyboard. In general, the tilde can be read as "modeled by", in this case "petal length modeled by species".
...
So far, we have learned about ANOVA in it's simplest form - i.e. to test for differences among means. But this is not the only way we can think about it and the clue is in the name - it analyses **variance**. What do we mean by this? To illustrate it, let's have a look at the **total** distribution of `Petal.Length` from the `iris` data. We can do this using a histogram (with `geom_histogram()`[^6]).

[^6]: Note how `geom_histogram()` only uses a single aesthetic (x). `ggplot` makes the y-axis for you, by counting the number of observations in each bin.
...
```{r read_data, message=FALSE, message = FALSE, results = "hide"}
...
Aha, the reason R says there are 2 phenotypes is because there are two columns in the phenotype data.frame - one called `id` and the other called `res`.[^8] This second variable is the resistance (i.e. S = susceptible, PR = partially resistant and R = resistant). We can extract the `res` column from `bedbugs$pheno` using _an additional_ `$`, and store this to an object.
...
[^8]: Incidentally, how did we know to use `$pheno` to access the phenotypic data? Using the `objects` function on the `bedbugs` object - i.e. `objects(bedbugs)` will give us a rundown of what we can access inside.
...
*`n.mar` - the number of markers
*`length` - the length of the linkage group in centimorgans (see below)
*`ave.spacing` - the average spacing between markers (in centimorgans)
*`max.spacing` - the maximum spacing between markers (in centimorgans)
...
A good way to summarise this data is to see it in a table. The following code summarises the association between different geno- and phenotypes for our locus. [^9]
...
[^9]: This code extracts the phenotype information and also the marker information for the QTL and used the function `factor` to turn them in to `factor` variables - where there is a label for each category of the data ([see here for a reminder of what a factor is](https://evolutionarygenetics.github.io/Introduction.html)). We then made everything into a `data.frame` for later. 

Now we have a table where we can see the numbers of each resistance phenotype for each genotype[^11]. It is fairly clear from this table and the figure if a bedbug has an A allele at this locus, it is more likely to be susceptible to the pesticide. In contrast, most BB individuals are resistant.
...
Last but not least. We can test the same association with ANOVA. To do this, we are going to read in some data [we prepared for you earlier](https://evolutionarygenetics.github.io/qtl_df.tsv)[^10].

[^10]: If you're curious, `stringsAsFactors = TRUE` is necessary here to explixitly tell R that the text columns are _factors_, a special way of encoding groups in R. We need to do this to turn the phenotypes into numeric values later. 
