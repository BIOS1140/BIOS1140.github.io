---
title: "Detecting Natural Selection"
output: 
...
This is part of what we call _vectorisation_, taking a single function and applying it to a vector of values.
...
The `apply()` function is a tool to further vectorise your functions that works with matrices and data frames. `apply()` will _apply a function to either each row or each column of your data frame/matrix_. The general structure of using `apply()` is:

```{r, eval = FALSE}
apply(data, row_or_colwise, function_name)
```

Where the second argument is either `1` for operating on each row or `2` for operating on each column[^1].

[^1]: It can be tricky to remember which is which of these, so don't worry if you find yourself looking at the help page with `?apply` all the time, I sure do! Also, a general rule of thumb is that in R, _rows always comes before columns_, like when you extract values from a data frame with square brackets `[]`. 
...
This should be the same as you got before, but with a lot less work. Next, we'll look at applying the function to a bigger data set with `apply()`. 
...
Great! So this works well. Now let's get $p$ (the frequency of the T allele) for all the populations. We can do using the `apply()` function that you learned about in the R-section. Note that the data needs to be columns 2 through 4 of the data, and the second argument needs to be `1` since we're working on rows.
...
[^2]: Tip: to better understand this code, try printing the objects `af_euram` and `af_eastasian`, as well as the columns `af_euram$p` and `af_eastasian$p`. This way you can follow what the code is doing.
