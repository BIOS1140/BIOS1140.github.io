...
a)  Use `calc_af()` together with `apply()` to calculate $p$ for each population. Add the values to `counts` as a column. *hint: you have to select only the numeric columns of the data frame to use* `apply()` *on it.*
