...
Take a look at `hominidae_dist` - you will see it is a matrix of the distance between the sequences. The distances are based on the number of nucleotide substitutions. How the numbers are calculated depend on the model we use - here, the F81. It is not straight forward to interpret the numbers directly in terms of how much the groups differ, but the larger the number the greater the genetic distance.
...
```{r, eval = TRUE, echo = FALSE, results = 'hidden', message = FALSE, warning = FALSE}
# read in the dog data
...
# read in village dog data
village_data <- read_delim("docs/data/village_dogs.tsv", delim = "\t")
```
